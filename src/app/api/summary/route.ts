...
export async function POST(request: NextRequest)
{
    try
    {
        ...
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 120000); // 2 minute timeout for summary
            ...
            openRouterResponse = await fetch("https://openrouter.ai/api/v1/chat/completions", {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: "system",
                            content: "Ты профессиональный аналитик текстов. Опиши кратко, о чем эта статья. Ответ должен быть на русском языке."
                        },
                        {
                            role: "user",
                            content: articleTitle 
                                ? `Опиши кратко, о чем эта статья:\n\nЗаголовок: ${articleTitle}\n\nСодержание:\n${articleContent}`
                                : `Опиши кратко, о чем эта статья:\n\n${articleContent}`
                        }
                    ],
                    temperature: 0.3
        ...
        if (!summary)
        {
            return NextResponse.json(
                { 
                    error: "Не удалось получить описание статьи. Пожалуйста, попробуйте еще раз.",
                    errorType: "API_ERROR"
                },
                { status: 500 }
            );
